...
struct ArrayCreationExpression : public Expression {
    VarPtr var;
    TypePtr elemType;
    ExpressionPtr sizeExpr;

    ArrayCreationExpression(Var *var, Type *elemType, Expression *sizeExpr, Location loc)
            : Expression{loc}, var{var}, elemType{elemType}, sizeExpr{sizeExpr} {}
...
struct ForStatement : public Statement {
  enum class Kind {
    ...
    ON,   // For loop over neighbour agents     for (Agent nx : on(env))
  };
  ...
  bool isOn() const { return kind == Kind::ON; }
  CallExpression &getOnCall() const {
    assert(isOn());
    return *dynamic_cast<CallExpression *>(&*expr);
  }
//  const Expression &getOnAgent() const { return getNearCall().getArg(0); }
  const Expression &getOnEnv() const { return getOnCall().getArg(0); }
};
...
struct SimulateStatement : public Statement {
  ...
  std::vector<FunctionDeclaration *> stepFuncDeclOnDevice[10];
...
struct ConflictResolutionStatement : public Statement {
    IdentListPtr tiebreakingFuncs;
    ExpressionPtr envExpr;
    ExpressionPtr radiusExpr;
    double radius;
    std::string envName;
    // Populated during analysis
    FunctionDeclaration * tiebreakingFuncDecl;

    ConflictResolutionStatement(Expression *envExpr, Expression *radiusExpr, IdentList *tiebreakingFuncs, Location loc)
            : Statement{loc}, tiebreakingFuncs{tiebreakingFuncs},envExpr{envExpr},radiusExpr{radiusExpr}  {}
...
struct AgentMember : public Node {
  ...
  AgentMember(bool isPosition, bool isArray, Type *type, std::string name, Location loc)
    : Node{loc}, isPosition{isPosition}, isArray{isArray}, type{type}, name{name} {}
...
struct AgentDeclaration : public Declaration {
  ...
  AgentDeclaration(std::string name,bool isRealAgent, AgentMemberList *members, Location loc)
    : Declaration{loc}, name{name}, isRealAgent{isRealAgent}, members{members} {}
  ...
  std::vector<AgentMember *> getMembers() const {
      std::vector<AgentMember *> ret;
        for (AgentMemberPtr &member : *members) {
...
struct Script : public Node {
  ...
  ConflictResolutionStatement *conrStmt = nullptr;
