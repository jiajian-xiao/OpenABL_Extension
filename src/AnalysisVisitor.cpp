...
void AnalysisVisitor::enter(AST::ArrayCreationExpression &) {}
...
void AnalysisVisitor::enter(AST::ConflictResolutionStatement &) {}
...
void AnalysisVisitor::leave(AST::AgentMember &decl) {
    if (decl.isArray) {
...
//    }
    ...
    else if (member->name == "env") {
      if (!expr.type.isArray()) {
        err << "Environment env must be Array" << expr.loc;
        return;
      }
      if (auto *exprVar = dynamic_cast<const AST::ArrayCreationExpression *>(&expr)) {
        decl.envType = exprVar->type.getAgentDecl()->name;
        decl.envName = exprVar->var.get()->name;
        decl.envGraphSize = evalExpression(*exprVar->sizeExpr);
      } else {
        err << "Environment env wrong" << expr.loc;
        return;
      }
...
//      currentFunc->accessedAgent = agent;
      stmt.kind = AST::ForStatement::Kind::ON;
...
void AnalysisVisitor::leave(AST::SimulateStatement &stmt) {
  ...
  for (const std::string &name : *stmt.stepFuncs) {
    if (isdigit(name[0]))
    {
        if (stmt.stepFuncDeclOnDevice[std::stoi( name )].size()<1)
            stmt.num_devices ++;

        AST::FunctionDeclaration* fn = stmt.stepFuncDecls.back();
        stmt.stepFuncDeclOnDevice[std::stoi( name )].push_back(fn);
        continue;
    }
    ...
    if (stmt.num_devices == 0)
        stmt.num_devices = 1;
  script.simStmt = &stmt;
};

void AnalysisVisitor::leave(AST::ConflictResolutionStatement &stmt) {
    if (script.conrStmt) {
      err << "Script can only contain a single conflict resolution statement" << stmt.loc;
      return;
    }
    if (stmt.tiebreakingFuncs->size() > 1) {
        err << "Conflict resolution can only have one tie-breaking function" << stmt.loc;
        return;
    }

    for (const std::string &name : *stmt.tiebreakingFuncs) {
      auto it = funcDecls.find(name);
      if (it == funcDecls.end()) {
        err << "Unknown tie-breaking function \"" << name << "\"" << stmt.loc;
        return;
      }

      AST::FunctionDeclaration &fn = *it->second;
      stmt.tiebreakingFuncDecl = &fn;
    }

    Value radiusVal = evalExpression(*stmt.radiusExpr);
    if (radiusVal.isFloat()) {
      stmt.radius = radiusVal.getFloat();
      radiuses.push_back(radiusVal);
    }
    else
      stmt.radius = 0;

    if (auto var = dynamic_cast<AST::VarExpression *>(&(*stmt.envExpr))){
        stmt.envName = var->var.get()->name;
    }

    script.conrStmt = &stmt;
};
...
void AnalysisVisitor::leave(AST::ArrayCreationExpression &expr) {
  Type elemType = expr.elemType->resolved;
  if (elemType.isArray()) {
    err << "Cannot instantiate nested array type" << expr.elemType->loc;
    return;
  ...
  expr.type = { Type::ARRAY, elemType };
  Value val = {};
  declareVar(*expr.var, expr.type, true, true, val);
